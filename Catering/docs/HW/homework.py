...
class OrderItem:
    ...
    def update(self, **payload) -> "OrderItem | None":
        ...
        if not row:
            ...
            self.order_id, self.dish_id, self.quantity = order_id, dish_id, quantity
            return self
    ...
    @classmethod
    def delete_with_filter(cls, **filters) -> bool:
        keys = filters.keys()
        conditions = " AND ".join([f"{key} = %s" for key in keys])
        values = tuple(filters.values())
        with DatabaseConnection() as db:
            db.cur.execute(
            f"DELETE FROM order_items WHERE {conditions} RETURNING id", values
        )
            return bool(db.cur.fetchall())
